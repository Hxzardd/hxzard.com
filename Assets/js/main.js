...
app.get('/host/upload', async (req, res) => {
  const response = await fetch('upload');
  const html = await response.text();
  document.querySelector('.wrapper').innerHTML = html;
});

app.get('/host/paste', async (req, res) => {
...
r.open("GET", "https://api.lanyard.rest/v1/users/930055165480951809"), r.send(), r.addEventListener("load", () => {
